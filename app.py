...
with st.sidebar:
    ...
    st.caption(
        "AI analysis is optional. Without an OpenAI API key the app creates "
        "a local draft storyboard so the workflow can still be tested."
    )
    ...
    model = st.text_input(
        "Analysis model",
        value=os.getenv("OPENAI_MODEL", "gpt-5.4-mini"),
    )

    st.divider()

    pexels_key = st.secrets.get(
        "PEXELS_API_KEY",
...
audio = st.file_uploader(
    ...
    help="The final video keeps this original audio track.",
)
...
st.caption(
    "Already have a storyboard from a prior creative review? "
    "Load its JSON here and continue directly to editing or animatic rendering."
)
...
if load_imported:
    if not audio:
        ...
        try:
            imported = json.loads(
                imported_storyboard_file.getvalue().decode("utf-8")
            )

            project_dir = Path(
                tempfile.mkdtemp(prefix="songstory_")
            )
            ...
            st.session_state.pop("pexels_results", None)
            st.session_state.pop("pexels_scene", None)
            st.session_state.pop("selected_pexels_clips", None)
            st.session_state.pop("final_video_path", None)
            st.session_state.pop("animatic_path", None)

            st.success(
                "Storyboard loaded. You can edit it or render "
                "the synchronized animatic below."
            )

        except Exception as exc:
            st.error(
                f"The storyboard JSON could not be loaded: {exc}"
            )
...
if analyze_clicked:
    if not audio:
        ...
        st.error(
            "Please paste or upload the lyrics for this first version of the app."
        )
        ...
        with st.spinner(
            "Interpreting the full song and planning the visual story…"
        ):
            if openai_key:
                try:
                    sb = ai_storyboard(
...
if sb:
    ...
    with b:
        ...
        st.text_area(
            "Visual continuity",
            value=sb.get("visual_style", ""),
            height=100,
            key="style_edit",
        )

    st.caption(
        f"Storyboard source: {sb.get('source', 'unknown')} • "
        f"Song duration: "
        f"{st.session_state.get('audio_duration', 0):.1f}s"
    )
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "lyric_excerpt": st.column_config.TextColumn(
                "Lyric / moment",
                width="medium",
            ),
    ...
    st.write(
        "This creates a complete MP4 using storyboard cards timed to the "
        "original song. It is a low-cost way to review pacing before "
        "selecting final footage."
    )

    if st.button(
        "Render animatic preview",
        type="primary",
    ):
        out = (
            Path(st.session_state.project_dir)
            / "songstory_animatic.mp4"
        )

        with st.spinner(
            "Rendering storyboard timing with the original song…"
        ):
            try:
                render_animatic(
                ...
                st.error(
                    f"Animatic render failed: {exc}"
                )

    if (
        st.session_state.get("animatic_path")
        and Path(st.session_state.animatic_path).exists()
    ):
        st.video(
            st.session_state.animatic_path
        )
    ...
    current_scenes = (
        st.session_state.storyboard.get("scenes", [])
    )

    if current_scenes:
        scene_numbers = [
            int(scene.get("scene", i + 1))
        ...
        selected = next(
            ...
            if int(scene.get("scene")) == selected_num
        )
        ...
        if st.button(
            ...
            if not pexels_key:
                st.error(
                    "The Pexels API key was not found "
                    "in Streamlit Secrets."
                )

            elif not search_query.strip():
                st.error(
                    "Enter a search description first."
                )

            else:
                with st.spinner(
                    "Searching Pexels for cinematic footage..."
                ):
                    try:
                        ...
                        st.session_state.pexels_scene = (
                            selected_num
                        )

                    except Exception as exc:
                        st.error(
                            f"Pexels search failed: {exc}"
                        )

        if (
            st.session_state.get("pexels_scene")
            == selected_num
        ):
            ...
            if not results:
                ...
                st.caption(
                    "Preview the choices below and select "
                    "the clip you prefer."
                )
        ...
        if chosen:
            st.success(
                f"Scene {selected_num} has "
                "a Pexels clip selected."
            )

    else:
        st.info(
            "No storyboard scenes are available yet."
        )


    st.subheader("7. Build the complete music video")

    selected_clips = st.session_state.get(
        "selected_pexels_clips",
        {},
    )

    required_scene_numbers = [
        int(scene.get("scene", i + 1))
        for i, scene in enumerate(current_scenes)
    ]
    ...
    selected_count = sum(
        1
        for scene_number in required_scene_numbers
        if scene_number in selected_clips
    )

    st.write(
        f"{selected_count} of {total_scenes} scenes "
        "have footage selected."
    )

    if (
        total_scenes
        and selected_count < total_scenes
    ):
        missing_scenes = [
            str(scene_number)
            for scene_number in required_scene_numbers
            if scene_number not in selected_clips
        ]

        st.info(
            "Select footage for every scene before building "
            "the complete video. Missing scenes: "
            + ", ".join(missing_scenes)
        )

    elif total_scenes:
        if st.button(
            "Build complete music video",
            type="primary",
        ):
            project_dir = Path(
            ...
            try:
                for index, scene_number in enumerate(
                    required_scene_numbers,
                    start=1,
                ):
                    clip = selected_clips[
                    ...
                    status.write(
                        f"Downloading footage for scene "
                        f"{scene_number}..."
                    )
                ...
                status.write(
                    "Assembling the complete music video..."
                )
                ...
                st.success(
                    "Your complete music video is ready."
                )

            except Exception as exc:
                st.error(
                    f"Final video build failed: {exc}"
                )
    ...
    if (
        final_video_path
        and Path(final_video_path).exists()
    ):
        st.video(
            final_video_path
        )
        ...
        st.download_button(
            "Download complete music video",
            Path(
                final_video_path
            ).read_bytes(),
            file_name="songstory_final.mp4",
            mime="video/mp4",
        )
